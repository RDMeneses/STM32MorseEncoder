...
This project implements a Morse code encoder on a STM32F411 Blackpill board that utilizes a buzzer (with PWM) to play Morse signals and an OLED display (from [BitDogLab](https://github.com/BitDogLab/BitDogLab)) to show each character as it is played. Instead of using UART communication, the project features three pre-defined messages that can be selected by the user through a button.
